-module('escript_with_wrong_arity_main_function@@main').
-export([run/1, main/1]).
...
%%% Used by escripts
main(_) ->
    run('escript_with_wrong_arity_main_function').

run_module(Module) ->
    try
        {ok, _} = application:ensure_all_started('escript_with_wrong_arity_main_function'),
